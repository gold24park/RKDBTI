...
.post<DatabaseRequest, NextApiResponse>(async (req, res) => {
    try {
      ...
      if (req.session?.uuid != null && typeNumber > 0) {
        let cached: number[] = JSON.parse((await redis.get(req.session.uuid)) || "[]")
        if (!cached.includes(typeNumber)) {
          ...
          // redis ttl = 10분
          await redis.set(req.session.uuid, JSON.stringify(cached), 'EX', 60 * 10)
        }
      } 
      return res.status(200).json({})
    } catch (e) {
